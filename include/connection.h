...
#include "cpr/cpr.h" // Using CPR library for HTTP requests
#include "nlohmann/json.hpp" // Using nlohmann/json for JSON handling
...
namespace subarulink {
  ...
  public:
    ...
    // Async methods returning std::future
  ...
  private:
    ...
    std::map<std::string, std::string> API_SERVER = {
...
// Implementation
  Connection::Connection(const std::string& username,
    ...
    // Set headers exactly as in Python
    _headers = {
        ...
        {"Origin", "file://"},
        {"X-Requested-With", API_MOBILE_APP[_country]},
  ...
  std::future<bool> Connection::_authenticate(const std::string& vin) {
    return std::async(std::launch::async, [this, vin]() {
      ...
      std::cout << "Debug: Headers being sent:" << std::endl;
      ...
      std::map<std::string, std::string> form_data = {
          ...
          {"password", _password},
          {"deviceId", _device_id},
          {"passwordToken", ""},  // empty string instead of null
          {"selectedVin", vin},
          {"pushToken", ""},      // empty string instead of null
          {"deviceType", "android"}
      };

      try {
        std::string endpoint = "https://" + API_SERVER[_country] + API_VERSION + "/login.json";
        std::cout << "Debug: Making authentication request to: " << endpoint << std::endl;

        // Pass empty json_data but include form_data
  ...
  std::future<nlohmann::json> Connection::_make_request(
    ...
    return std::async(std::launch::async, [this, url, method, headers, params, data, json_data, baseurl]() {
      std::string base = baseurl.empty() ?
                         "https://" + API_SERVER[_country] + API_VERSION : baseurl;
      ...
      if (method == "POST") {
        if (!data.empty()) {  // Handle form data first
          ...
          // Handle JSON data (but we're not using this for authentication)
